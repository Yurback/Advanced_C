...
int (*dispatchTable[])(int, int) = {add, sub, mult, div};

// performs the given operation on elements of the arrays
int *performOp(int *, int *, int, int (*)(int, int));
...
int main()
{
	...
	while (choice != 5)
	{
		...
		result = performOp(array1, array2, size, dispatchTable[j]);
...
int *performOp(int *a, int *b, int size, int (*pf)(int, int))
{
	int *newarr = malloc(size * sizeof(int)); // СОЗДАВАТЬ int newarr[] нельзя и потом возвращать return &newarr, т.к. все что создается в фукции пропадает
